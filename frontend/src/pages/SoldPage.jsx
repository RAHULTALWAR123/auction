...
const SoldPage = () => {
    ...
    const {fetchPlayer,player,getOwner,owner} = usePlayerStore()
    const {pid} = useParams();

    useEffect(() => {
        fetchPlayer(pid);
        getOwner(pid);
    },[fetchPlayer,pid,getOwner])
  ...
  return (
    ...
    <div className='border border-gray-600 rounded-2xl h-auto w-96 mt-5 text-center flex flex-col items-center justify-center py-7 bg-gradient-to-t from-white to-gray-800'>
        ...
        <p className='text-xl font-bold text-black mt-4'>Sold to : {owner?.username}</p>
...
<Link to={`/category/${player?.category}`}>
    <button>
